...
class PageAction
{
    ...
    protected $callback;
    /** @var string[] */
    protected array $menuSlugs  = [];
    protected string $innerPage = '';
    /** @var bool|string */
     ...
     * @return string
     */
    public function getUrl(array $extraData = [], bool $relative = true): string
    {
        $data = [
     ...
     * @return string
     */
    public function getActionNonceFileds(bool $echo = true): string
    {
        ob_start();
        wp_nonce_field($this->getNonceKey());
        echo '<input type="hidden" class="foldsnap-action-form-action" name="' . esc_attr(ControllersManager::QUERY_STRING_MENU_KEY_ACTION)
     ...
     * @return boolean
     */
    public function isPageOfCurrentAction(array $currentMenuSlugs): bool
    {
     ...
     * @return boolean
     */
    public function isCurrentAction(array $currentMenuSlugs, string $currentInnerPage, string $action): bool
    {
        if ($action !== $this->key) {
            return false;
        }
    ...
    {
        $nonce = SanitizeInput::str(SanitizeInput::INPUT_REQUEST, '_wpnonce');
     ...
     *
     * @param array<string,mixed> $resultData generic array where put additional action data
     *
     * @return bool
     */
    public function exec(array &$resultData = []): bool
    {
        $result = true;
        try {
            ...
            $resultData['errorMessage'] = $errorMsg . '<b>' . esc_html($e->getMessage()) . '</b><pre>' . esc_html($e->getTraceAsString()) . '</pre>';
            $result                     = false;
        }
        return $result;
    }
